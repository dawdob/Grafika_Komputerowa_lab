...
public class SceneGraph extends JPanel {
    ...
    private static abstract class SceneGraphNode {
        ...
        abstract void polygon(Graphics2D g);
    }
    ...
    private static SceneGraphNode filledTriangle = new SceneGraphNode() {
        void polygon(Graphics2D g) { // width = 1, height = 1, center of base is at (0,0);
    ...
    private static SceneGraphNode filledPolygon = new SceneGraphNode() {
        ...
        void polygon(Graphics2D g2) {
            int[] x1 = new int[14];
            int[] y1 = new int[14];
            double r = 150;
            double val = 0;
            double k = (Math.PI * 2) / 14;


            for (int i = 0; i < 14; i++) {
                x1[i] = (int) (r * Math.sin(val));
                y1[i] = (int) (r * Math.cos(val));
                val += k;
            }
            Polygon polygon = new Polygon(x1, y1, 14);
